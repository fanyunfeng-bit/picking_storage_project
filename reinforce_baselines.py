...
class WarmupBaseline(Baseline):
    ...
    def eval(self, x, c):
        ...
        # self.alpha 可以看做baseline和warmup_baseline的权重
        if self.alpha == 1:
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            if self.problem.NAME != 'para' and self.problem.NAME != 'r_para':
