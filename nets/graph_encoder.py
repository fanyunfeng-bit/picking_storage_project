...
class GraphAttentionEncoder(nn.Module):
    def __init__(
        ...
        if self.hierarchy_block != None:
            ...
            self.cluster_layers = MultiHeadAttentionLayer(n_heads, embed_dim, self.hierarchy_block, feed_forward_hidden,
                                                          normalization)

    def forward(self, x, mask=None, return_transform_loss=False, is_vrp=False, is_para=False):
        ...
        if self.input_transform:
            ...
            t_input = t_input.view(batch_size, 1, 4) + self.input_transform_biases.view(1, 1, 4).repeat(batch_size, 1,
                                                                                                        1)
        ...
        if self.hierarchy_block != None:
            ...
            while torch.max(distance) > radius:
                ...
                distance_new_cluster = torch.sqrt(torch.sum((x[torch.max(distance, dim=1)[0] > radius] -
                                                             new_cluster[torch.max(distance, dim=1)[0] > radius]) ** 2,
                                                            2))
            ...
            hmean = torch.sum(h_cluster * (cluster_center[:, :, 0] != -1).float().view(batch_size, cluster_size, 1),
                              dim=1)
