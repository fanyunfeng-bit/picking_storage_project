...
def generate_para_data(dataset_size, ss_size, r_size):
    ...
    for i in range(dataset_size):
        ...
        while len(locs) < (ss_size + r_size):
            ...
            if [row, col] not in locs:
                locs.append([row, col])
        robot = np.zeros(2)
        shelf_loc = locs[:ss_size]
        return_loc = locs[ss_size:]

        station_index = np.random.choice(np.arange(len(stations)), ss_size, replace=True)
        station_loc = (stations[station_index])

        ss_loc = np.concatenate((shelf_loc, station_loc), axis=-1)
        robot_locs.append(robot)
        ss_locs.append(ss_loc)
        r_locs.append(return_loc)
    robot_locs = np.array(robot_locs)
    ss_locs = np.array(ss_locs)
    r_locs = np.array(r_locs)
    # print(ss_locs)

    return list(zip(
        robot_locs.tolist(),  # robot initial location
        ss_locs.tolist(),  # shelf-station locations
        r_locs.tolist()  # station locations
    ))
...
def generate_vrp_data(dataset_size, vrp_size):
    ...
    return list(zip(
        np.random.randint(size=(dataset_size, 2)).tolist(),  # Depot location
        ...
        if problem != 'para':
            ...
            if opts.filename is None:
                filename = os.path.join(datadir, "{}{}_{}_{}_seed{}.pkl".format(
                    problem,
                    "_{}".format(opts.ss_size), opts.r_size, opts.name, opts.seed))
            else:
                filename = check_extension(opts.filename)
            ...
            np.random.seed(opts.seed)
            dataset = generate_para_data(opts.dataset_size, opts.ss_size, opts.r_size)
