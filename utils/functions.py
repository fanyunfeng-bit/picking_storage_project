...
def load_problem(name):
    from problems import TSP, CVRP, SDVRP, OP, PCTSPDet, PCTSPStoch, PARA, RPARA
    problem = {
        'tsp': TSP,
        'cvrp': CVRP,
        'sdvrp': SDVRP,
...
def save_dataset_excel(dataset):
    if dataset is not None:
        assert os.path.splitext(dataset)[1] == '.pkl'

        with open(dataset, 'rb') as f:
            data = pickle.load(f)

    workbook = xlwt.Workbook()
    worksheet_robot = workbook.add_sheet('robot')
    worksheet_shelf = workbook.add_sheet('shelf')
    worksheet_station = workbook.add_sheet('station')
    worksheet_return = workbook.add_sheet('return')
    # title = [U'Para', U'dataset']

    # worksheet.write_row('A1', title)
    print(data[0])
    for i in range(0, len(data)):
        # data_ = [u'robot', data[i-1][0][0]]
        worksheet_robot.write(i, 0, data[i][0][0])
        worksheet_robot.write(i, 1, data[i][0][1])
        for j in range(10):
            worksheet_shelf.write(i, 2*j, data[i][1][j][0])
            worksheet_shelf.write(i, 2*j+1, data[i][1][j][1])
            worksheet_station.write(i, 2*j, data[i][1][j][2])
            worksheet_station.write(i, 2*j+1, data[i][1][j][3])
        for j in range(10):
            worksheet_return.write(i, 2*j, data[i][2][j][0])
            worksheet_return.write(i, 2*j+1, data[i][2][j][1])
        # worksheet.write(row, 1, [u'robot', data[i-1][0][1]])
